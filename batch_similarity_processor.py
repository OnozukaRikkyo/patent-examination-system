...
class BatchConfig:
    ...
    def __init__(self):
        ...
        self.PROJECT_ROOT = "llm/csv1"
        self.csv_path = f"{self.OUTPUT_ROOT_DIR}/graph/csv1/ref/class_cos_sim1.csv"
...
import copy
def process_single_row(row, index, verbose=False):
    """
    CSVの1行を処理して類似度を計算
...
def process_batch(config, start_index=None, end_index=None, overwrite=False):
    ...
    for i in range(start, end):
        ...
        try:
            row = df.iloc[i]
            success, msg = process_single_row(row, i)
            if success:
                processed_count += 1
            else:
                error_count += 1
